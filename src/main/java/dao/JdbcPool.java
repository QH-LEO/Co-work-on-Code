...
/**
 * Created by andilyliao on 16-7-14.
 */
...
public class JdbcPool {
    ...
    public static String testTable = ""; // 测试连接是否可用的测试表名，默认没有测试表
    public static int initialConnections = 10; // 连接池的初始大小
    public static int incrementalConnections = 5; // 连接池自动增加的大小
    private static int maxConnections = 50; // 连接池最大的大小
    public static Vector connections = null; // 存放连接池中数据库连接的向量 , 初始时为 null
    // 它中存放的对象为 PooledConnection 型
    public JdbcPool(String jdbcDriver, String dbUrl, String dbUsername,
                    String dbPassword) {
    ...
    public static void createConnections(int numConnections) throws SQLException {
        // 循环创建指定数目的数据库连接
        for (int x = 0; x < numConnections; x++) {
            ...
            System.out.println(" 数据库连接己创建 ......"+connections.size());
        }
    }
    ...
    public  synchronized static Connection getConnection() throws SQLException {
        // 确保连接池己被创建
        if (connections == null) {
            System.out.println("pool invaild");
        ...
        while (conn == null) {
            ...
            // 则表明创建一批连接后也不可获得可用连接
            System.out.println("wait and incresing");
        }
    ...
    public static Connection  getFreeConnection() throws SQLException {
        // 从连接池中获得一个可用的数据库连接
    ...
    public static Connection findFreeConnection() throws SQLException {
    ...
    public static boolean testConnection(Connection conn) {
        try {
            ...
            if (testTable.equals("")) {
                ...
                //check if this aonnection is valid
